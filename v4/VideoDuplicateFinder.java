...
public class VideoDuplicateFinder extends JFrame {
    ...
    public VideoDuplicateFinder() {
        ...
        ffprobePanel.setVisible(matchByLength.isSelected());

        matchByLength.addActionListener(e -> ffprobePanel.setVisible(true));
        matchBySize.addActionListener(e -> ffprobePanel.setVisible(false));
        matchByName.addActionListener(e -> ffprobePanel.setVisible(false));
        ...
        new SwingWorker<List<List<File>>, String>() {
            protected List<List<File>> doInBackground() {
                ...
                if (mode.equals("length")) {
                    return groupByDuration(allFiles, ffprobePath);
                } else {
            ...
            private List<List<File>> groupByDuration(List<File> files, String ffprobe) {
                ...
                // Group files within ±1 second using sorted sweep
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    for (int j = i + 1; j < n; j++) {
                        ...
                        if (d - baseDur > 1.0) break; // sorted, so no more matches
